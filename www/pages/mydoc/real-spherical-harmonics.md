...
where the normalized associated Legendre functions for use with the $$4\pi$$-normalized spherical harmonics are given by
...
and the spherical harmonics are orthogonal for all degrees $$l$$ and orders $$m$$
...
SHTOOLS supports the use of $$4\pi$$-normalized, Schmidt semi-normalized, orthonormalized, and unnormalized spherical harmonic functions. To specify which normalization should be used, it is only necessary to specify the optional parameter `norm` in the Fortran 95 or `normalization` in the Python routines:
