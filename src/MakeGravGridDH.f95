subroutine MakeGravGridDH(cilm, lmax, gm, r0, a, f, rad_grid, theta_grid, phi_grid, total_grid, n, sampling, lmax_calc, omega, &
		normal_gravity, pot_grid)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!	Given the gravitational spherical harmonic coefficients CILM, this subroutine
!	will compute a 2D Driscol and Healy sampled grid of the three components and magnitude 
!	of the gravity in SI units. The output grids are cacluated on a flattened ellipsoid with semi-major 
!	axis A and flattening F. The output grids contain N samples in latitude and longitude by default, but if 
!	the optional parameter SAMPLING is set to 2, the grids will contain N samples in latitude and 2N samples 
!	in longitude. In order to calculate the entire gravitational acceleration, it is necessary that the degree-0 term 
!	be set equal to 1. Radial gravity is assumed to be positive when directed UPWARDS. If the optional parameter 
!	OMEGA is specified, the gravitational acceleration will be calculated in the reference frame 
!	of a rotating body. If the parameter NORMAL_GRAVITY is set to 1, the normal gravity predicted for a 
!	flattened ellipsoid with A, F, and OMEGA will be removed from the magnitude of the total acceleration.
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	real*8 :: 		grid(4*lmax+4), pi, theta, scalef, rescalem, u, p, dp, pmm, &
				...
				coeft0, coefts0, coefp0, coefps0, coefus0, b
	...
	if (present(normal_gravity)) then
		if (normal_gravity == 1) then
			...
			do i=1, n
				...
				total_grid(i,1:nlong) = total_grid(i,1:nlong) - NormalGravity(lat, GM, omega, a, b)
			enddo
		endif
	endif
