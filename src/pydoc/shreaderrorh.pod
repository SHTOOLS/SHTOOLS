...
I<SHReadErrorH> will read spherical harmonic coefficients from an ASCII file into an array CILM. The maximum spherical harmonic degree that is read is determined by the minimum of the dimension of the input array CILM - 1 and the maximum degree of the coefficients in the file. If the optional array SKIP is specified, parsing of the file will commence after the first SKIP lines. If the optional array HEADER is specified, then the first N elements after the skipped lines will be output, where N is the length of the array HEADER.
