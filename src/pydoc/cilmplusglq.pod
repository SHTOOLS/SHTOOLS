...
=item SUBROUTINE CilmPlusGLQ ( 

CILM, GRIDIN, LMAX, NMAX, MASS, D, RHO, W, ZERO )
...
The weights used in the Gauss-Legendre quadrature, which are required for GRIDTYPE = 1. These are calculated from a call to `SHGLQ` (or alternatively, I<PreGLQ>).
