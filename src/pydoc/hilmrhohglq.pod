...
=item SUBROUTINE HilmRhoHGLQ ( 

CILM, BA, GRID, LMAX, NMAX, MASS, R0, RHO, W, ZERO, FILTER_TYPE, FILTER_DEG, LMAX_CALC )
...
CILM(2, LMAX+1, LMAX+1), BA(2, LMAX+1, LMAX+1), GRID(NLAT, NLONG), MASS, R0, RHO(NLAT, NLONG), ZERO(LMAX+1), W(LMAX+1)
...
The spherical harmonic bandwidth of the input relief GRID. If LMAX_CALC is not set, this determines also the maximum spherical harmonic degree of the output spherical harmonic coefficients of the relief and the input spherical harmonics of the Bouguer anomaly. As a general rule, LMAX should be about twice the real spherical harmonic bandwidth of the input function to avoid aliasing.
...
The weights used in the Gauss-Legendre quadrature. These are calculated from a call to `SHGLQ` (or alternatively, I<PreGLQ>). If present, one of PLX or ZERO must also be present.
...
Apply a filter when calculating the relief in order to minimize the destabilizing effects of downward continuation which amplify uncertainties in the Bouguer anomaly. If 0 (default), no filtering is applied. If 1, use the minimum amplitude filter of Wieczorek and Phillips (1998; equation 19). If 2, use a minimum curvature filter. 
...
hilm(1), shexpandglq(1), makegridglq(1), shglq(1), preglq(1), shexpanddh(1), makegriddh(1), glqgridcoord(1)
