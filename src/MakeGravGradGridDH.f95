...
!	will compute 2D Driscol and Healy sampled grids of the six components of the 
...
!		(Vzx,	Vzy,	Vzz)
!	
!	where X points NORTH, Y points WEST, and Z points UPWARD. The gravitational potential is defined as
!
!		V = GM/r Sum_{l=0}^LMAX (R0/r)^l Sum_{m=-l}^l C_{lm} Y_{lm}, 
!
!	Laplaces equation implies that Vxx + Vyy + Vzz = 0, and the gravity tensor is symmetric. The 
!	components are calculated according to eq. 1 in Petrovskaya and Vershkov (2006, J. Geod, 80, 117-127), 
!	which is based on the eq. 3.28 in Reed (1973, Ohio State Univ., Dept. Geod. Sci., Rep. 201, Columbus, OH). 
!	Note that Reed's equations are in terms of latitude, and that the Y axis points East.
!
!		Vzz = Vrr
...
!
!	where r, t, p stand for radius, theta, and phi, and subscripts on V denote partial derivatives.
!
!	The output grid are in units of 1/s and are cacluated on a flattened ellipsoid with semi-major 
!	axis A and flattening F. To obtain units of Eotvos (10^-9 s^-1), multiply by 10^9. The output 
!	grids contain N samples in latitude and longitude by default, but if 
!	the optional parameter SAMPLING is set to 2, the grids will contain N samples in latitude and 2N samples 
!	in longitude. The first latitudinal band of the grid corresponds to 90 N, the latitudinal band for 90 S is not calculated, 
...
!			GM:		Product of the gravitatonal constant and the planet's mass
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	real*8 :: 		grid(4*lmax+4), pi, theta, scalef, rescalem, u, p, dp, dp2, dp2s, pmm, sint, &
				pm1, pm2, z, tempr, r_ex, lat, prefactor(lmax), coefr0, coefrs0, &
				coeft0, coefts0, coefp0, coefps0, coefrr0, coefrrs0, coefrt0, coefrts0, &
				coefrp0, coefrps0, coeftp0, coeftps0, coefpp0, coefpps0, coeftt0, coeftts0
	complex*16 ::		coef(2*lmax+3), coefr(2*lmax+3), coefrs(2*lmax+3), &
				coeft(2*lmax+3), coefts(2*lmax+3), coefp(2*lmax+3), coefps(2*lmax+3), &
				tempc, coefrr(2*lmax+3), coefrrs(2*lmax+3), coefrt(2*lmax+3), coefrts(2*lmax+3), &
				coefrp(2*lmax+3), coefrps(2*lmax+3), coeftp(2*lmax+3), coeftps(2*lmax+3), &
				coefpp(2*lmax+3), coefpps(2*lmax+3), coeftt(2*lmax+3), coeftts(2*lmax+3)
	integer*8 ::		plan
	...
	if (size(vxx(:,1)) < n .or. size(vxx(1,:)) < nlong .or. size(vyy(:,1)) < n .or. size(vyy(1,:)) < nlong &
		.or. size(vzz(:,1)) < n .or. size(vzz(1,:)) < nlong .or. size(vxy(:,1)) < n .or. &
		size(vxy(1,:)) < nlong .or. size(vxz(:,1)) < n .or. size(vxz(1,:)) < nlong &
		.or. size(vyz(:,1)) < n .or. size(vyz(1,:)) < nlong) then
		print*, "Error --- MakeGravGradGridDH"
		if (present(sampling)) then
			if (sampling == 1) then
				print*, "VXX, VYY, VZZ, VXY, VXZ, and VYZ must be dimensioned as (N, N) where N is ", n
			elseif (sampling == 2) then
				print*, "VXX, VYY, VZZ, VXY, VXZ, and VYZ must be dimensioned as (N, 2N) where N is ", n
			endif
		else
			print*, "VXX, VYY, VZZ, VXY, VXZ, and VYZ must be dimensioned as (N, N) where N is ", n
		endif
		print*, "Input dimensions are ", size(vxx(:,1)), size(vxx(1,:)), size(vyy(:,1)), size(vyy(1,:)), &
			size(vzz(:,1)), size(vzz(1,:)), size(vxy(:,1)), size(vxy(1,:)), size(vxz(:,1)), size(vxz(1,:)), &
	...
	if (lmax_comp /= lmax_old) then
		...
		if (sum(astat(1:4)) /= 0) then
			print*, "MakeGravGradGridDH --- Error"
	...
	do i=1, i_eq - 1, 1
		...
		if (lmax_comp /= 0) then	! l = 1
			...
			tempr = cilm(1,2,1) * pm1 * (6) * prefactor(1)	! 6 = (l+1)*(l+2) prefactor
			coefrr0 = coefrr0 + tempr
			...
			! dp2 is the second derivative with respect to THETA. Must multiply dp by -sin(theta) in the recurrence relation
			! note that the first term is symmetric about the equator accoring the fsymsign, whereas the second term differs
			! by -1.
			dp2 = -2 * pm1 + z * dp
			dp2s = 2 * pm1 + z * dp
		...
		do m = 1, lmax_comp-1, 1
			...
			tempc = dcmplx(cilm(1,m1,m1), - cilm(2,m1,m1)) * pm2 * (m+1) * (m+2) * prefactor(m)	! (m,m)
			...
			tempc = -dcmplx(cilm(1,m1,m1), - cilm(2,m1,m1)) * pm2 * prefactor(m) * m**2 ! (m,m)
			coefpp(m1) = coefpp(m1) + tempc
			coefpps(m1) = coefpps(m1) + tempc ! fsymsign = 1
			
			tempc = dcmplx(cilm(2,m1,m1), cilm(1,m1,m1)) * pm2 *(-m-1) * prefactor(m) * m ! (m,m)
			...
			tempc = dcmplx(cilm(1,m1+1,m1), - cilm(2,m1+1,m1)) * pm1 * (m+2) * (m+3) * prefactor(m+1)	! (m+1,m)
	   		coefrr(m1) = coefrr(m1) + tempc	
			coefrrs(m1) = coefrrs(m1) - tempc ! fsymsign = -1
			...
			tempc = -dcmplx(cilm(1,m1+1,m1), - cilm(2,m1+1,m1)) * pm1 * prefactor(m+1) * m**2	! (m+1,m)
	   		coefpp(m1) = coefpp(m1) + tempc	
			coefpps(m1) = coefpps(m1) - tempc ! fsymsign = -1
			
			tempc = dcmplx(cilm(2,m1+1,m1), cilm(1,m1+1,m1)) * pm1 * (-m-2) * prefactor(m+1) * m	! (m+1,m)
	   		coefrp(m1) = coefrp(m1) + tempc	
			coefrps(m1) = coefrps(m1) - tempc ! fsymsign = -1
			...
			tempc = dcmplx(cilm(1,m1+1,m1), - cilm(2,m1+1,m1)) * dp * (-m-2) * prefactor(m+1)	! (m+1,m)
	   		coefrt(m1) = coefrt(m1) + tempc	
			coefrts(m1) = coefrts(m1) + tempc	! reverse fsymsign
			
			tempc = dcmplx(cilm(2,m1+1,m1), cilm(1,m1+1,m1)) * dp * prefactor(m+1) * m	! (m+1,m)
	   		coeftp(m1) = coeftp(m1) + tempc	
			coeftps(m1) = coeftps(m1) + tempc ! reverse fsymsign
			
			dp2 = -(m1*(m1+1) - (m**2)/u**2) * pm1 + z * dp
			dp2s = (m1*(m1+1) - (m**2)/u**2) * pm1 + z * dp
			tempc = dcmplx(cilm(1,m1+1,m1), - cilm(2,m1+1,m1)) * prefactor(m+1)	! (m+1,m)
	   		coeftt(m1) = coeftt(m1) + tempc	 * dp2
			coeftts(m1) = coeftts(m1) + tempc * dp2s
			...
			do l = m+2, lmax_comp, 1
				...
				tempc = dcmplx(cilm(1,l1,m1), - cilm(2,l1,m1)) * p * (l1) * (l1+1) * prefactor(l)
				coefrr(m1) = coefrr(m1) + tempc
				...
				tempc = -dcmplx(cilm(1,l1,m1), - cilm(2,l1,m1)) * p * prefactor(l) * m**2
				coefpp(m1) = coefpp(m1) + tempc
				coefpps(m1) = coefpps(m1) + tempc * fsymsign(l1,m1)
				
				tempc = dcmplx(cilm(2,l1,m1), cilm(1,l1,m1)) * p * (-l1) * prefactor(l) * m
				coefrp(m1) = coefrp(m1) + tempc
				coefrps(m1) = coefrps(m1) + tempc * fsymsign(l1,m1)
				
				dp = ( sqr(2*l+1) * sqr(l-m) * sqr(l+m) / sqr(2*l-1) * pm1 - z * l * p) / u**2
      				tempc = dcmplx(cilm(1,l1,m1), - cilm(2,l1,m1)) * dp * prefactor(l)
				coeft(m1) = coeft(m1) + tempc
				coefts(m1) = coefts(m1) - tempc * fsymsign(l1,m1) ! reverse fsymsign
				
				tempc = dcmplx(cilm(1,l1,m1), - cilm(2,l1,m1)) * dp * (-l1) * prefactor(l)
				coefrt(m1) = coefrt(m1) + tempc
				coefrts(m1) = coefrts(m1) - tempc * fsymsign(l1,m1)	! reverse fsymsign
				
				tempc = dcmplx(cilm(2,l1,m1), cilm(1,l1,m1)) * dp * prefactor(l) * m
				coeftp(m1) = coeftp(m1) + tempc
				coeftps(m1) = coeftps(m1) - tempc * fsymsign(l1,m1)	! reverse fsymsign
				
				dp2 = -(l*l1 -(m**2)/u**2) * p + z * dp
				dp2s = -(l*l1 -(m**2)/u**2) * p * fsymsign(l1,m1)  - z * dp * fsymsign(l1,m1) 
		...
		if (lmax_comp /= 0) then
			...
			coefrts(lmax_comp+1) = coefrts(lmax_comp+1) - tempc ! reverse fsymsign
			
			tempc = dcmplx(cilm(2,lmax_comp+1,lmax_comp+1), cilm(1,lmax_comp+1,lmax_comp+1)) * dp &
				 * prefactor(lmax_comp) * lmax_comp
			...
			tempc = dcmplx(cilm(1,lmax_comp+1,lmax_comp+1), - cilm(2,lmax_comp+1,lmax_comp+1)) &
				* prefactor(lmax_comp)
		...
		! Note that the first angular derivatives are with repsect to z, but that the second is with respect to theta.
		...
		if (present(sampling)) then
			if (sampling == 2) then
				coef(lmax+2:2*lmax+3) = dcmplx(0.0d0,0.0d0)	
               	...
               	if (i==1) then
               		vyy(1,1:nlong) = 0.0d0	! These derivatives are undefined at the pole
               		vzz(1,1:nlong) = 0.0d0	
               		vxy(1,1:nlong) = 0.0d0	
               		vxz(1,1:nlong) = 0.0d0	
               		vyz(1,1:nlong) = 0.0d0	
			...
			if (present(sampling)) then
				if (sampling == 2) then
					coef(lmax+2:2*lmax+3) = dcmplx(0.0d0,0.0d0)	
               		...
               		! Vyy = 1/r Vr + 1/r^2 /tan(t) Vt + 1/r^2 /sin(t)^2 Vpp
              	 	coef(1) = dcmplx(coefr0/r_ex + coeft0/(r_ex**2)/tan(theta) + coefpp0/(r_ex**2)/u**2 ,0.0d0)
			coef(2:lmax+1) = (coefr(2:lmax+1)/r_ex + coeft(2:lmax+1)/(r_ex**2)/tan(theta) + &
			...
			if (present(sampling)) then
				if (sampling == 2) then
					coef(lmax+2:2*lmax+3) = dcmplx(0.0d0,0.0d0)	
               		...
               		! Vxy = 1/r^2 /sin(t) Vtp - cos(t)/sin(t)^2 /r^2 Vp
               		coef(1) = dcmplx(coeftp0/sint/r_ex**2 - coefp0/(r_ex**2)*z/u**2, 0.0d0)
			coef(2:lmax+1) = (coeftp(2:lmax+1)/sint/r_ex**2 - coefp(2:lmax+1)/(r_ex**2)*z/u**2 )/2.0d0
			if (present(sampling)) then
				if (sampling == 2) then
					coef(lmax+2:2*lmax+3) = dcmplx(0.0d0,0.0d0)	
				endif
			...
			coef(2:lmax+1) = (coeft(2:lmax+1)/r_ex**2 - coefrt(2:lmax+1)/r_ex )/2.0d0
			if (present(sampling)) then
				if (sampling == 2) then
					coef(lmax+2:2*lmax+3) = dcmplx(0.0d0,0.0d0)	
				endif
			...
			coef(2:lmax+1) = (coefp(2:lmax+1)/sint/r_ex**2 - coefrp(2:lmax+1)/sint/r_ex )/2.0d0
			if (present(sampling)) then
				if (sampling == 2) then
					coef(lmax+2:2*lmax+3) = dcmplx(0.0d0,0.0d0)	
				endif
			...
			coef(2:lmax+1) = (coefrs(2:lmax+1)/r_ex + coeftts(2:lmax+1)/r_ex**2 )/2.0d0
			if (present(sampling)) then
				if (sampling == 2) then
					coef(lmax+2:2*lmax+3) = dcmplx(0.0d0,0.0d0)	
				endif
               		...
               		! Vyy = 1/r Vr + 1/r^2 /tan(t) Vt + 1/r^2 /sin(t)^2 Vpp
              	 	coef(1) = dcmplx(coefrs0/r_ex + coefts0/(r_ex**2)/tan(theta) + coefpps0/(r_ex**2)/u**2 ,0.0d0)
			coef(2:lmax+1) = (coefrs(2:lmax+1)/r_ex + coefts(2:lmax+1)/(r_ex**2)/tan(theta) + &
			...
			if (present(sampling)) then
				if (sampling == 2) then
					coef(lmax+2:2*lmax+3) = dcmplx(0.0d0,0.0d0)	
               		...
               		! Vxy = 1/r^2 /sin(t) Vtp - cos(t)/sin(t)^2 /r^2 Vp
               		coef(1) = dcmplx(coeftps0/sint/r_ex**2 - coefps0/(r_ex**2)*z/u**2, 0.0d0)
			coef(2:lmax+1) = (coeftps(2:lmax+1)/sint/r_ex**2 - coefps(2:lmax+1)/(r_ex**2)*z/u**2 )/2.0d0
			if (present(sampling)) then
				if (sampling == 2) then
					coef(lmax+2:2*lmax+3) = dcmplx(0.0d0,0.0d0)	
				endif
			...
			coef(2:lmax+1) = (coefts(2:lmax+1)/r_ex**2 - coefrts(2:lmax+1)/r_ex )/2.0d0
			if (present(sampling)) then
				if (sampling == 2) then
					coef(lmax+2:2*lmax+3) = dcmplx(0.0d0,0.0d0)	
				endif
			...
			coef(2:lmax+1) = (coefps(2:lmax+1)/sint/r_ex**2 - coefrps(2:lmax+1)/sint/r_ex )/2.0d0
			if (present(sampling)) then
				if (sampling == 2) then
					coef(lmax+2:2*lmax+3) = dcmplx(0.0d0,0.0d0)	
				endif
	...
	do m = 1, lmax_comp-1, 1
		...
		dp2 = -(m*m1 -(m**2)) * pm2 
		tempc = dcmplx(cilm(1,m1,m1), - cilm(2,m1,m1)) * dp2 * prefactor(m)	! (m,m)
	...
	if (lmax_comp /= 0) then
		...
		tempc = dcmplx(cilm(1,lmax_comp+1,lmax_comp+1), - cilm(2,lmax_comp+1,lmax_comp+1)) * dp2 &
			* prefactor(lmax_comp)
	...
	coef(2:lmax+1) = (coefr(2:lmax+1)/r_ex + coefpp(2:lmax+1)/(r_ex**2) )/2.0d0
	if (present(sampling)) then
		if (sampling == 2) then
			coef(lmax+2:2*lmax+3) = dcmplx(0.0d0,0.0d0)	
		endif
	...
	coef(2:lmax+1) = (coefp(2:lmax+1)/r_ex**2 - coefrp(2:lmax+1)/r_ex )/2.0d0
	if (present(sampling)) then
		if (sampling == 2) then
			coef(lmax+2:2*lmax+3) = dcmplx(0.0d0,0.0d0)	
		endif
