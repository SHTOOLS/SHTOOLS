...
!-------------------------------------------------------------------------------
    ...
    if (def == 1) then
        if (present(taper_wt)) then
            ...
            call SHBiasK (tapers, lwin, K, sgt, lmax, sgt_bias(1:lmax_calc+1), &
                            taper_wt = taper_wt, save_cg = 1)
            call SHBiasK (tapers, lwin, K, sgg, lmax, sgg_bias(1:lmax_calc+1), &
                            taper_wt = taper_wt, save_cg = 1)
            call SHBiasK (tapers, lwin, K, stt, lmax, stt_bias(1:lmax_calc+1), &
                            taper_wt = taper_wt, save_cg = 1)
    ...
    elseif (def == 2) then
        do i = 1, K
            ...
            admit(1:lmax_calc+1) = admit(1:lmax_calc+1) &
                            + sgt_bias(1:lmax_calc+1) / stt_bias(1:lmax_calc+1)
