subroutine HilmRhoH(cilm, ba, grid, lmax, nmax, mass, r0, rho, gridtype, w, &
...
!-------------------------------------------------------------------------------
    ...
    do l = 1, lmax_calc2
        cilm(1:2,l+1,1:l+1) = filter(l+1) * ba(1:2,l+1,1:l+1) * mass * &
                            dble(2*l+1) * ( (r0/d)**l) / (4.0d0*pi*d**2)
    end do
    ...
    do n = 2, nmax
        ...
        if (gridtype == 1) then
            if (present(plx)) then
                ...
                call SHExpandGLQ(cilmn, lmax, grid2(1:nlat,1:nlong), &
                                w(1:lmax+1), zero = zero(1:lmax+1), norm = 1, &
