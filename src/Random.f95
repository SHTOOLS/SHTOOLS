...
!   shift sequence. Returns a uniform random deviate between 0.0 and 1.0 
!   (exclusive of the endpoint values). This fully portable scalar generator has
!   the "traditional" (not Fortran 90) calling sequence with a random deviate as
...
!$OMP   threadprivate(am, ix, iy, k)
    ...
    iy = IA * (iy - k * IQ)-IR * k
    if (iy < 0) iy = iy + IM
    RandomN=am * ior(iand(IM, ieor(ix,iy)), one)    ! Combine the two generators 
                                                    ! with masking to
...
!$OMP   threadprivate(gset, gaus_stored)
    ...
    if (gaus_stored) then
        ...
        rsq = sqrt(-2.0d0*log(rsq)/rsq)     ! Now make the Box-Muller 
                                            ! transformation to get
                                            ! two normal deviates. Return one 
