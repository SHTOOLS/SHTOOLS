...
def interpolate_2dgrid(x_new, y_new, x1d_old, y1d_old, grid_old, order=2):
    ...
    # add north and south pole values to the grid
    ...
    grid_old_with_poles[0, :] = _np.mean(grid_old[0, :])
    grid_old_with_poles[-1, :] = _np.mean(grid_old[-1, :])

    # add extra poles to the x=lat coordinates
    x1d_old_wrap = _np.concatenate([[-90.], x1d_old, [90.]])
    ix1d_old_wrap = _np.arange(nx1d + 2)

    # add wrapped longitude to y=lon coordinates
    y1d_old_wrap = _np.concatenate([[y1d_old[-1] - 360.], y1d_old,
                                    [y1d_old[0] + 360.]])
    iy1d_old_wrap = _np.concatenate([[ny1d - 1], _np.arange(ny1d),
                                     [0]])

    # find index coordinate of flattened x_new, y_new
    ix = _np.interp(x_new.flat, x1d_old_wrap, ix1d_old_wrap)
    iy = _np.interp(y_new.flat, y1d_old_wrap, iy1d_old_wrap)

    # make index grid
    points = _np.array([ix, iy])

    # get flattened values from grid
    values = _map_coordinates(grid_old_with_poles, points, order=order,
                              mode='wrap')
